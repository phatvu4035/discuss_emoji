...
(function($) {
	jQuery.mbEmoticons= {
		...
		addSmilesBox:function(){
			$(this).each(function(){
				...
				$.each($.mbEmoticons.smiles,function(i){
					var emoticon=$("<span/>").addClass("emoticon").html(i).attr("title",i).attr('emoji-id', $.mbEmoticons.smiles_emoji_id[i]);
		...
		openSmileBox:function(){
			var content_message_parent = $(this).parents('.o_thread_message_content');
			var textarea = $(this);
			var smilesBox= textarea.data("smilesBox");
			var right= 20;
			var trigger_button_height = $(this).height();
			var content_message_parent_height = content_message_parent.outerHeight()
			
			smilesBox.css({right:right, top:(-1 * content_message_parent_height) + 30});
			smilesBox.fadeIn();
			setTimeout(function(){
	...
	jQuery.fn.emoticonize = function (animate) {
		function convert (text){
			...
			return text.replace (icons,function(str){
				...
				if (ret){
					ret="<img src='"+'./discuss_emoji/static/lib/jquery-emotionicons/'+"smiley/"+ret+ext+"' align='absmiddle'>";
					return ret;
				}else{
					return str;
				}
